<template>
  <div class="big-menu" v-if="menuBig">
    ...
    <div class="big-menu__sub-list-menu" v-if="showSubList">
      <div class="filtering">
        <div class="pull--left">
          <b-form-input :disabled="isDisableFilter" @keyup.enter.native="onFilterKeyUp" size="sm" placeholder="Filter" style="font-size: 12px" v-model="filter"></b-form-input>
        </div>
        <!-- <div class="pull--right" v-b-toggle="'menuCollapseAll'+indexEvent"> -->
          <div class="pull--right">
          <span class="filtering__ex-col when-closed" style="font-size: 12px" @click="collapseAll">Collapse All</span> |
          <span class="filtering__ex-col when-opened" style="font-size: 12px" @click="expandAll">Expand All</span>
        ...
        >
          ...
          <!-- <b-collapse :id="'menuCollapseAll'+indexEvent"> -->
            <b-collapse :visible="collapsed[indexEvent]" :id="'menuCollapseAll'+indexEvent">
              ...
              >
                <div class="list__name" @click.self="doCallMenu(option)">
                  {{option.menu_name}}
                </div>
                <!-- kalau Fav === 0 maka ga muncul bintang -->
                <div v-if="option.Fav === 0" class="list__icon-right">
                  <i class="icon-star" @click="doInsert(event.Menu_Seq_No, option.OptionSeq, option.PS, indexChild)"></i>
                </div>
                <!-- selain 0 dia muncul bintang -->
                <div v-else class="list__icon-right is-favorite">
                  <i class="icon-star" @click="doDelete(event.Menu_Seq_No, option.OptionSeq, indexChild)"></i>
                </div>
              </div>
            </b-collapse>
          <!-- </b-collapse> -->
      ...
      <div class="box-lists" v-show="onFiltered">
        ...
        >
          ...
          <div v-if="option.Fav === '0'" class="list__icon-right">
            <i class="icon-star" @click="doInsert(option.Menu_Seq_No, option.OptionSeq, option.PS, index)"></i>
          </div>
          <div v-else class="list__icon-right is-favorite">
...
export default {
  ...
  created: function() {
    ...
    this.modules = this.menus === undefined ? undefined : this.menus.ModuleList
    this.events = this.menus === undefined ? undefined : this.menus.EventWithChild
  },
  updated: function () {
  ...
  methods: {
    ...
    getMenuBy () {
      ...
      this.postEncode( this.getUrlGetMenuBy(), param ).then(response => {
          // response from API
    ...
    doInsert: function(menuSeqNo, optionSeq, ps, indexChild) {
      ...
      this.postEncode(this.getUrlInsertFavorite(), param)
      .then((response) => {
        if(response == null) return

        for (var i = 0; i < this.menus.EventWithChild.length; i++) {
          if (this.menus.EventWithChild[i].Menu_Seq_No == menuSeqNo) break
        }
        
        this.menus.EventWithChild[i].Child[indexChild].Fav = 1

        localStorage.lsMenu = JSON.stringify(this.menus)
        this.events = this.menus.EventWithChild

        localStorage.lsFavoriteMenu = JSON.stringify(response.Data)
        this.$parent.$children[1].$children[2].changeFavMenu(localStorage.lsFavoriteMenu)
      })
    },
    doDelete: function(menuSeqNo, optionSeq, indexChild) {
      ...
      this.postEncode(this.getUrlDeleteFavorite(), param)
      .then((response) => {
        if(response == null) return
        ...
        this.menus.EventWithChild[i].Child[indexChild].Fav = 0

  ...
  <div style="padding-left: 15px;padding-right: 15px;">
    ...
    <div class="modal-customize-abs__modal-header" style="padding-top:0px !important; padding-bottom:0px !important;">
      <div class="modal-customize-abs__modal-header--title" style="width: 97% !important;">
        <b-row style="margin-bottom: 5px;">
          <b-col class="search-toolbar">
            <b-form-input
              ...
              style="background-color: white;margin-bottom: 0px !important;margin-top: 3px !important; padding-left:30px !important; border-top: none; border-right: none; border-left: none; min-height: 40px; border-radius: 0px !important; width:82% !important;"
              @keyup.enter.native="onSearchEnter"
              :disabled="inAdvance"
              autocomplete="off">
            </b-form-input>
            <span style="text-align: right; width: 110px; margin-left: 81%;font-size: 12px; color: #007bff !important; position: absolute !important; left: 17px !important; margin-top: 11px !important; cursor: pointer !important; text-decoration: underline;" @click="onAdvance">{{advanceText}}</span>
        ...
        <b-row style="padding-bottom:0px" v-show="inAdvance">
          <b-col sm="4">
            <b-form-select
              v-model="columnName"
              :options="fieldFilter"
          ...
          <b-col sm="4">
            <b-form-input
              v-if="inAdvance && signFilters[signFilters.map(x => x.value).indexOf(tandanya)].type=='text'"
              v-model="filteran"
              type="text"
              placeholder="Enter a value"
              size="sm"
...
export default {
  ...
  methods: {
    onAdvance() {
      if (this.inAdvance) {
        this.advanceText = 'Advance Filter'
      }
      else {
        this.advanceText = 'Close Advance Filter'
      }

      this.inAdvance = !this.inAdvance

      if (this.inAdvance == false) {
        this.columnName = 1
        this.tandanya = '='
        this.filteran = ''
    ...
    //     EventBus.$emit("ebProcessPostAndDelete" + this.prop.PageLevel + "_" + this.prop.TabIndex, { Act: act, Data: data })
    //   } else {
    //     if (act == "post") {
    ...
    //     } else if (act == "process") {
    //       this.alertWarning("No Data Selected To Process");
    //     }
    //   }
    // },
    // M_Delete(data) {
    ...
    headClicked: function(field, index) {
      ...
      field = this.fieldFilter[this.fieldFilter.map(x => x.text).indexOf(field)].realCol
      this.sort = "";
      var x = dtSort
        .map(e => {
          return e.field;
    ...
    doGetList(search, method) {
      ...
      if (this.inAdvance) {
        var theFilter = ''
        // var field = this.fieldFilter.map((data, index) => {
        //   // console.log(index, data)
        //   return data.value;
        // })

        // console.log(field, this.columnName)

        // var idxField = field.indexOf(this.columnName)

        // console.log(idxField)

        // var icolumn = this.fieldFilter[this.fieldFilter.map(x => x.value).indexOf(this.columnName)].realCol
        if (this.tandanya == "in" || this.tandanya == "notin") {
          var val = this.filteran.split(",");
          var xVal = "";
          val.forEach(v => {
            xVal += "'" + v + "',";
          });
        ...
        } else if (this.tandanya == "like") {
          theFilter += " like '%" + this.filteran + "%'";
        } else if (this.tandanya == "null") {
          theFilter += " is null ";
        } else if (this.tandanya == "notnull") {
          theFilter += " is not null ";
        } else {
          theFilter += " " + this.tandanya + " '" + this.filteran + "'";
        }

        var src = ''
        src = this.columnName && this.columnName != '' ? (this.fieldFilter[this.fieldFilter.map(data => data.value).indexOf(this.columnName)].realCol + theFilter) : ''
        temp = temp == "" ? src : temp + ' AND ' + src
      }
      ...
      var param = {
        ...
        PortfolioCd: this.getDataUser().PortfolioCd,
        SubPortfolioCd: this.getDataUser().SubPortfolioCd,
        LookUpCd: this.dataLookUp.LookUpCd,
        ColumnDB: this.dataLookUp.ColumnDB,
        PerPage: this.perPage,
        CurrentPage: this.currentPage,
        InitialWhere: temp,
        ParamWhere: !this.inAdvance ? this.searchValue : '',
        SortField: this.sort,
        ParamView: this.dataLookUp.ParamView
      };

      if (this.dataLookUp.SourceField == undefined || this.dataLookUp.DisplayLookUp == undefined ||
          this.dataLookUp.SourceField == '' || this.dataLookUp.DisplayLookUp == '') {
      } else {
        param.SourceField = this.dataLookUp.SourceField
        param.DisplayLookUp = this.dataLookUp.DisplayLookUp
      }
      ...
      var urls = this.isTAnalysis ? this.urlApi :this.getUrlLookupList()
      var params = this.isTAnalysis && urls == this.getUrlTanalysisLookup() ? paramT :param
      this.postEncode(urls, params).then(response => {
        ...
        for (var j = 0; j < keys.length; j++) {
          if (keys[j].includes('=')) {
            this.fieldFilter.push({
              value: j+1,
              realCol: keys[j].split("=")[1].trim(),

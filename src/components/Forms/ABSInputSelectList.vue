  ...
  <span id="ABSSelectList">
    ...
    <b-row v-if="prop.cVisible && prop.cIsAlwaysShow">
      ...
      <b-col>
        ...
        <span
          v-show="inputStatus!=='view' && errors.has(prop.cParentForm+'.'+prop.cName)"
          class="error-span"
    ...
    <b-row v-else-if="prop.cVisible">
      <b-col :md="prop.cLabelSize" class="lbl-col-align">
        <template v-if="languageStatus">
          <label v-show="inputStatus=='new' || inputStatus=='edit'" class="lbl-text-form">
            <span v-if="isRequired" style="color:red;">*</span>
            {{ $t(prop.cName) == prop.cName ? prop.cLabel : $t(prop.cName)}}
          </label>
      ...
      <b-col v-if="prop.cMasterUrl!=undefined" md="1">
        <div
          class="add-icon--plus"
          v-show="inputStatus!=='view' && prop.cMasterUrl!=undefined"
          @click="doShowMaster"
        >
          <i class="icon-plus"></i>
        </div>
...
export default {
  ...
  methods: {
    ...
    doShowMaster() {
      ...
      console.log("parameter loop", this.prop.cParamUrl);
    ...
    masterUrlPopUp2(url, title, w, h, from) {
      ...
      console.log(
        url + "&isPopup=true&from=" + from,
        title,
        "toolbar=0,scrollbars=yes, resizable=no,location=no, width=" +
          w +
          ", height=" +
          h +
          ", top=" +
      ...
      // newWindow.document.getElementById("content-app").style.paddingLeft =
      //   "0px !important";
      // }, 2000)

      // var sideBar = false;
      // this.hideSideBarMenu(sideBar);
      ...
      newWindow.onbeforeunload = function(from) {
        ...
        if (dataHidden.length > 0) {
          ...
          pHidden.value =
            '{"id":"","label":"","_from":"' + c + '","isLookUpOpen":0}';
        }
        pHidden.dispatchEvent(new Event("change"));
        this.$store.commit("handlePaddingLeftContent", "0px");
      };
      if (window.focus) {
        newWindow.focus();
      }
    ...
    getData() {
      ...
      // this.prop.dataLookUp.UserId = this.getDataUser().UserId
      // this.prop.dataLookUp.PortfolioCd = this.getDataUser().PortfolioCd
      // this.prop.dataLookUp.SubPortfolioCd = this.getDataUser().SubPortfolioCd

      this.$refs.modalLookup.$children[0].dataLookUp.UserId = this.getDataUser().UserId;
      this.$refs.modalLookup.$children[0].dataLookUp.PortfolioCd = this.getDataUser().PortfolioCd;
      this.$refs.modalLookup.$children[0].dataLookUp.SubPortfolioCd = this.getDataUser().SubPortfolioCd;
    ...
    checkPropDisplayColumn() {
      if (this.prop.cDisplayColumn) {
        this.alertWarning(
          "Please check prop cDisplayColumn on " + this.prop.cLabel
        );
        return;
      }
    }
  ...
  mounted() {
    ...
    this.prop.cParamMasterUrl =
      this.prop.cParamMasterUrl == "" ? undefined : this.prop.cParamMasterUrl;

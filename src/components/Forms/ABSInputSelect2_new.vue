  ...
  <span id="ABSSelectAsycn">
    ...
    <b-row v-show="prop.cVisible">
      ...
      <b-col>
        ...
        <label class="lbl-value-view-form notranslate" v-show="inputStatus=='view'">{{selected?selected.label:''}}</label>
      </b-col>
      <b-col md="1">
        <div class="add-icon--plus" v-show="inputStatus!=='view' && prop.cMasterUrl!=undefined && prop.cLastLabel==undefined" @click="doShowMaster">
          <i class="icon-plus"></i>
        </div>
    ...
    selected: {
      get: function(el) {
        ...
        if (this.value == "" || this.value == null) {
          ...
          if (this.isCanChange) {
            if (typeof this.value == 'object') {
              return this.options[this.options.map(x => x.id).indexOf(this.value.id.toString())]
            }
            else {
              return this.options[this.options.map(x => x.id).indexOf(this.value.toString())]
            }
          }
          else {
            if (typeof this.value == 'object') {

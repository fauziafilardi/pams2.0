<template>
  <b-row class="header-toolbar">
    ...
    <b-col lg="9" md="9" sm="12">
      ...
      />
      <!-- {{ statusFunction[2].disabled+ "||" + isNew+ "||" + isCannotDelete+ "||" + !deleteStatus }} -->
      <!-- ||isCannotDelete -->
      <ABSButton
        text="Save"
        icon="save"
      ...
      <ABSButton
        ...
        classIcon="icon-style-1"
        @click="doPost"
        :disabled="statusFunction[4].disabled||isNew||isCannotDelete||!postStatus"
      />
      <!-- {{ statusFunction[4].disabled + '||' + isNew + '||' + isCannotDelete + '||' + !postStatus }} -->
    ...
    <b-col class="search-toolbar" v-show="!getIsPopup()">
      <b-form-input
        id="txtSearch"
        v-model="search"
        type="text"
...
export default {
  computed: {
    ...
    postStatus () {
      return this.$store.getters.getPostStatus
    }
    // isByPassCancel () {
  ...
  methods: {
    ...
    getChild () {
      if (this.isProcess == true) {
        ...
        // console.log(this.pageLevel, this.tabIndex)
        if (this.pageLevel == 1) {
          return this.$parent.$parent.$children[this.pageLevel].$children[0].$children[0].$children[0]
        }
        else {

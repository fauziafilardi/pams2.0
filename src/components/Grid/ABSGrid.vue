      ...
      >
        ...
        <template v-for="(data, index) in prop.comp" :slot="data.slot" slot-scope="row">
          ...
          <template v-if="data.compType=='groupingLabel'">
            ookoko
            <template v-if="row.index == 0">{{row.item[data.slot]}}</template>
            <template
              v-else-if="row.item[data.slot] != items[row.index - 1]"
            >{{row.item[data.slot]}}</template>
          </template>
          <template v-else-if="data.compType=='ABSTextBoxOnly'">
            ...
            <b-col v-show="inputStatus!=='view'">
              <b-form-input
                ...
                :id="data.cName"
                :rowLine="row.index"
                :disabled="(inputStatus=='edit' && data.cKey) || data.cProtect"
                v-model="row.item[data.slot]"
                type="text"
            ...
            <b-col
              md
              v-if="inputStatus=='view' && data.cType=='decimal'"
...
export default {
  ...
  methods: {
      ...
      if (index > -1) {
        if (x < 0) {
          if (this.isCheckAsStatus != undefined) {
            if (
              this.items[index].hasOwnProperty(
                this.isCheckAsStatus.split("=")[0]
              )
            ) {
              this.items[index][
                this.isCheckAsStatus.split("=")[0]
              ] = this.isCheckAsStatus.split("=")[1];
            }
          }
    ...
    doGetList(search, method) {
      ...
      var param;
      if (
        this.prop.propGetDataBy == undefined ||
        this.prop.propGetDataBy == null
      ) {
        param = {
      ...
      // this.loader = true;
      this.postEncode(
        this.prop.propGetDataBy == undefined || this.prop.propGetDataBy == null
          ? this.getUrlList()
          : this.getUrlById() + this.prop.NoGetById,
        param
      ).then(response => {
        // this.loader = false;
      ...
      if (this.fieldDecimal.length != 0) {
        for (var l = 0; l < this.items.length; l++) {
          ...
          if (this.prop.cTotalColumn != "") {
            for (var n = 0; n < this.prop.cTotalColumn.split(",").length; n++) {
              ...
              if (this.total[n] != undefined || this.total[n] != null) {
                var valBefore = parseFloat(
                  this.replaceAllString(this.total[n], ",", ""),
                  this.decimal
                );
                valBefore = valBefore ? valBefore : 0;
                this.total[n] =
                  parseFloat(valBefore, this.decimal) +
                  parseFloat(ttl, this.decimal);
              } else {
          ...
          for (var m = 0; m < this.fieldDecimal.length; m++) {
            ...
            var data = (this.items[l][this.fieldDecimal[m]] = this.items[l][
              this.fieldDecimal[m]
            ]);
            ...
            var field = parseFloat(
              this.replaceAllString(
                this.items[l][this.fieldDecimal[m]],
                ",",
                ""
              ),
              this.decimal
            ).toFixed(this.decimal);
            this.items[l][this.fieldDecimal[m]] = this.isCurrency(
              field,
              this.decimal
            );
          }
    ...
    formatNumber(evt, indexField, slot, index) {
      ...
      if (
        this.prop.comp[indexField].cType == "decimal" ||
        this.prop.comp[indexField].cType == "numeric"
      ) {
        // console.log('lala')
        ...
        if (this.items[index][slot] !== "") {
          ...
          if (num.toString().indexOf(".") > -1) {
            ...
            if (numSplit[1].length < this.prop.comp[indexField].cDecimal) {
              ...
              var x = parseFloat(num).toFixed(
                this.prop.comp[indexField].cDecimal
              );
          ...
          this.items[index][slot] =
            numReal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
            numDecimal;
  ...
  mounted: function() {
    if (this.prop.divStyle == undefined || this.prop.divStyle == null) {
      ...
      this.prop.divStyle =
        "max-height: " + this.prop.divStyle + "px;overflow-y: scroll;";
    }

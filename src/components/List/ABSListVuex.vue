      ...
      >
        <template slot="HEAD_chkBoxAction" slot-scope="data">
          <b-form-checkbox
            ...
            v-model="selected"
            :disabled="isDisableTable  || (isCheckDisable == undefined ? false : isCheckDisable)"
          />
        </template>
...
export default {
  ...
  methods: {
      ...
      this.filteredColumn.forEach((filter, index) => {
        ...
        if (sign == "in" || sign == "notin") {
          ...
          if (filter.value.includes('*')){
            // this.replaceAllString(filter.value, '*', '%', 'string')
            // filter.value.replace(new RegExp("\\*\\"), '%')
            theFilter += "like '" + this.replaceAllString(filter.value, "*", "%", "string") + "'"
          }
          else {
    ...
    headClicked: function(field, index) {
      ...
      if (field.toUpperCase() == "NO" || field.toUpperCase() == "CHKBOXACTION") {
        return
      }
    ...
    headChkBoxAction() {
      ...
      this.$emit("allCheckBoxAction", this.items, this.selected);
    },
    chkBoxAction(index) {
    ...
    rowClicked: function(record, index) {
      ...
      this.$store.commit('SetPage' + this.prop.PageLevel + 'Data', this.items[index])
    ...
    doGetList(search, method, idx = null) {
      // this.checkOrderBy()
      ...
      if (method != undefined) {
        if (method == "save" || method == "update" || method == "refresh" || method == "fromChild" || method == "eb_getList") {
          temp = this.prop.initialWhere
          this.sortedField = []
          if (this.hasOrderBy) {
            // this.sort = this.prop.OrderBy
            var ss = this.prop.OrderBy.split(' ')
            this.sortedField.push({field: ss[0], sort: ss[1]})
          }
          else {
            this.sort = "TimeEdit DESC"
      ...
      this.postEncode(this.getUrlList(), param).then(response => {
        ...
        if (method == 'fromChild' || method == 'fromChild1') {
          // console.log('cuk')
          if (idx !== null) {
            this.rowSel = idx;
            this.items[idx]["_rowVariant"] = "primary";
            this.$store.commit('SetPage' + this.prop.PageLevel + 'Data', this.items[idx])
            // this.$emit("rowClicked", this.items[idx], idx)
            // this.rowClicked(this.items[idx], idx)
            // this.$emit("rowClicked", record, idx)
            this.$parent.getDataBy(this.items[idx])
          }
        }
        ...
        for (var i = 0; i < str_array.length; i++) {
          ...
          if (str_array[i].toLowerCase().includes('amount') || str_array[i].toLowerCase().includes('amt') || str_array[i].toLowerCase().includes('rate') || str_array[i].toLowerCase().includes('price')) {

    ...
    methods: {
        ...
        getAllColumn (search) {
            ...
            var param = {
                ...
                LineNo: 0,
                InitialWhere: "(AccountCd like '%" + search + "%' OR Descs like '%" + search + "%')",
                SortField: '',
                ParamView: ""
            }
            ...
            .then( response => {
                ...
                for (var x = 0; x < this.allDataOptions.length; x++) {
                    ...
                    for (var y = 0; y < this.rightDataOptions.length; y++) {
                        if (this.allDataOptions[x].AccountCd == this.rightDataOptions[y].value) {
                            status = true
                            break
                        }
                    }
                    if (status == false) {
                        this.leftDataOptions.push({
                            value: this.allDataOptions[x].AccountCd,
                            key: this.allDataOptions[x].AccountCd + ' - ' + this.allDataOptions[x].Descs,
            ...
            .then( response => {
                ...
                for (var x = 0; x < data.length; x++) {
                    this.rightDataOptions.push({
                        value: data[x].AccountCd,
                        key: data[x].AccountCd + ' - ' + data[x].Descs,
                        RowId: data[x].RowId,
                    })
                }
                this.getAllColumn('')
        ...
        M_Save () {
            if (this.leftDataOptions.length <= 0) {
               // insert data
                for (var x = 0; x < this.rightDataOptions.length; x++) {
                    dataToSave.push({
                        AccountCd: this.rightDataOptions[x].value,
                        // ModuleCd: 'AR',
                        SubPortfolioCd: this.getDataUser().SubPortfolioCd,
                ...
                for (let x = 0 ; x < this.leftDataOptions.length ; x ++) {
                    param1.push({
                        ...
                        _LineNo_: 2,
                        SubportfolioCd: this.getDataUser().SubPortfolioCd,
                        AccountCd: this.leftDataOptions[x].value
                    })
                }

  ...
  <div class="height--full">
    <div class="header--content">
      ...
      <div class="content-app-form__body" style="background-color: white;padding-top: 1px !important;margin-left: 15px;margin-right: 15px;height: 95%;overflow-y: auto;overflow-x: hidden;padding-bottom: 1%;">
        ...
        <b-row style="margin-left:20px;margin-top: 20px;padding-left:30px;">					
          <b-col md="12" id="col-left">
            <b-form :data-vv-scope="'FormScope_' + 1 + '_' + 1" :data-vv-value-path="'FormScope_' + 1 + '_' + 1">
            <b-row v-for="(dataRow) in RowData" :key="dataRow">
                ...
                <template v-if="dataRow == data.RowNo">
                  <template v-if="data.isColumn == 'True'">
                    ...
                    <template v-if="data.ColumnType=='Label'">
                      <b-col md="6"  :offset="index > 1 ? ((response[index-1].RowNo != data.RowNo && parseInt(data.position) % 2 == 0) ? '6' : '') : ''">
                        <span>
                          <b-row>
                            <b-col md="7" offset="4" style="text-align: justify;">
                    ...
                    <template v-if="data.ColumnType=='String'">
                      <b-col md="6"  :offset="index > 1 ? ((response[index-1].RowNo != data.RowNo && parseInt(data.position) % 2 == 0) ? '6' : '') : ''">
                        <ABSinputTextVuex
                          :prop="{
                            cValidate: (data.MaxLength && data.MaxLength != '' && data.MaxLength != 0 ? 'max:' + data.MaxLength + '|' : '') + data.isRequired,
                            cName: data.ParamName,
                            cLabel: data.Label,
                            cLabelSize: '4',
                            cOrder: data.TabOrder,
            ...
            <div v-show="isTExist" class="div-collapse" >
              <span class="master-span-form">
                {{TagForT}}
              </span>
            </div>
            <ABSTAnalysisFromTo v-show="isTExist" :prop="PTAnalysis" v-model="M_TAnalysis" ref="ref_ReportAnalysis" />
...
export default {
  ...
  methods: {
                    ...
                    for (let iw = 0; iw < splitInitialWhere.length; iw++) {
                      if (splitInitialWhere[iw+1]){
                        if (splitInitialWhere[iw+1].toUpperCase().includes('AND')) {
                          if (splitInitialWhere[iw].split('=')[0].trim().toUpperCase() == cols.toUpperCase()) {
    ...
    M_GenerateReport () {
      ...
      // console.log(this.M_TAnalysis)
      // console.log(this.ColumnT)

      var TVariable = this.M_TAnalysis.map(x => x[this.ColumnT])
      ...
      this.$validator._base.validateAll(scopeForm).then((result) => {
        ...
        .then((ress) => {
          ...
          for (let idx = 0; idx < this.response.length; idx++) {
            var yesno = 0
            var isDT = 0
            // console.log(this.response[idx].IncludeParameter)
            if (this.response[idx].IncludeParameter && this.response[idx].IncludeParameter != '') {
              var par = this.response[idx].IncludeParameter.split('=')[0]
              var val = this.response[idx].IncludeParameter.split('=')[1]
              ...
              // console.log(this.models, this.models.hasOwnProperty(par))
              
              if (this.models.hasOwnProperty(par)) {
                if (this.models[par] !== val) {
                  // return
                  continue
                  // break
                }
            ...
            if (this.response[idx].isParam == 'True') {
              ...
              var T_idx = TVariable.indexOf(this.response[idx].ParamName.substring(0,2))

              if (T_idx > -1) {
                //   console.log(this.response[idx].ParamName)
                //   TVariable
                if (this.response[idx].ParamName.substring(2).toUpperCase().includes('FROM')) {
                    this.models[this.response[idx].ParamName] = this.M_TAnalysis[T_idx].TValueFrom ? this.M_TAnalysis[T_idx].TValueFrom : ''
                }
                else if (this.response[idx].ParamName.substring(2).toUpperCase().includes('TO')) {
                    this.models[this.response[idx].ParamName] = this.M_TAnalysis[T_idx].TValueTo ? this.M_TAnalysis[T_idx].TValueTo : ''
                }
              }
            }
            // if (this.response[idx].isParam == 'True') {
    ...
    loadData() {
        ...
        .then((response) => {
          ...
          this.response = null
          this.response = response.Data.HTML

          this.RowData = [...new Set(response.Data.HTML.map(x => x.RowNo))]
          ...
          for (let a = 0; a < this.response.length; a ++) {
            ...
            this.response[a].isRequired = this.response[a].isRequired && this.response[a].isRequired.toString().toUpperCase() === 'TRUE' ? 'required' : ''
          ...
          if (this.response[0].AnalysisType && this.response[0].AnalysisType != '') {
              this.PTAnalysis.cPageLevel = 1
              this.PTAnalysis.cTabIndex = 1
              this.PTAnalysis.cAnalysisType = this.response[0].AnalysisType
              this.TagForT = this.response[0].AnalysisType
              this.PTAnalysis.isManual = true
              this.PTAnalysis.optionData = response.Data.DataAnalysis

              if (this.response[0].AnalysisType == 'Account Analysis') {
                this.ColumnT = 'AccountAnalysisCategory'
              }
              else if (this.response[0].AnalysisType == 'Analysis Category' || this.response[0].AnalysisType == 'Analysis Category All') {
                this.ColumnT = 'TrxAnalysisCategory'
              }
              else if (this.response[0].AnalysisType == 'Stock Analysis') {
                this.ColumnT = 'StockAnalysisCategory'
              }
              else if (this.response[0].AnalysisType == 'Debtor Analysis') {
                this.ColumnT = 'DebtorAnalysisCategory'
              }
              else if (this.response[0].AnalysisType == 'Creditor Analysis') {
                this.ColumnT = ''
              }
              else if (this.response[0].AnalysisType == 'Item Analysis') {
                this.ColumnT = 'ItemAnalysisCategory'
              }
              else if (this.response[0].AnalysisType == 'Supplier Analysis') {
                this.ColumnT = 'SupplierAnalysisCategory'
              }
              else if (this.response[0].AnalysisType == 'Lot Analysis') {
                this.ColumnT = 'LotAnalysisCategory'
              }
              else if (this.response[0].AnalysisType == 'Tag Analysis') {
                this.ColumnT = 'TagAnalysisCategory'
              }
              else if (this.response[0].AnalysisType == 'Fixed Asset Analysis') {
                this.ColumnT = 'FixedAssetAnalysisCategory'
              }

              // this.$forceUpdate()
              this.$refs.ref_ReportAnalysis.getData()
              this.isTExist = true
          }
        })
    },

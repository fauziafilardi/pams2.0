          ...
          <b-col sm="12">
            ...
            <div class="modal-customize-abs__modal-body">
              <treeselect
                ...
                :options="floorList"
                :clearable="false"
                placeholder="Choose Floor Plan"
                v-model="floorChoosed"
              />
              <ABSButton
                text="Save"
                icon="save"
    ...
    <vl-map ref="map"
      ...
      <vl-layer-vector ref="vectorLayer" id="my-features">
        <vl-source-vector ref="vectorSource">
          <vl-feature v-for="feature in floorList"
                      :key="feature.id"
                      :id="feature.id"
    ...
    watch: {
      ...
      selectedFeatures: function(newData, oldData) {
        ...
        if (this.selectedFeatures.length > 0) {
          ...
          var desc = ''
          for (var x = 0 ; x < this.floorList.length ; x ++) {
            if (newData[0].id == this.floorList[x].id) {
              desc = this.floorList[x].Descs
              break
            }
          }
          this.$parent.$parent.masterPlanChoosed = 'FP'+ this.floorList[x].id
          ...
          //     }
          //     this.postEncode(this.getUrlUpdateCoordinateFloorPlan(), param)
          //     .then((response) => {
    ...
    methods: {
      ...
      changeStatus () {
        if (this.captionButton == 'Add Marker') {
          ...
          this.floorChoosed = null
          this.$refs.modalFloor.show()
        }
      },
      refreshMap () {
      ...
      getFloorPlan () {
        ...
        this.postEncode(this.getUrlGetCoordinateFloorPlan(), param)
        .then((response) => {
          if (response == null) return

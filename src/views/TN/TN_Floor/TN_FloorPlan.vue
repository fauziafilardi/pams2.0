<template>
  <div>
          ...
          <b-col sm="12">
            ...
            <div class="modal-customize-abs__modal-body">
              ...
              <ABSButton
                ...
                @click="updateCoordinateFloor"
                :disabled="unitChoosed == null"
              />
              <br />
            </div>
          </b-col>
    ...
    <vl-map ref="map"
            ...
            style="height: 500px;border: solid 1px gray;"
      ...
      <vl-layer-vector ref="vectorLayer" id="my-features">
        <vl-source-vector ref="vectorSource">
          <vl-feature v-for="feature in unitList"
                      :key="feature.id"
                      :id="feature.id"
...
<script>
  ...
  export default {
      ...
      selectedFeatures: function(newData, oldData) {
        ...
        if (this.selectedFeatures.length > 0) {
          ...
          for (var x = 0 ; x < this.unitList.length ; x ++) {
            if (newData[0].id == this.unitList[x].id) {
              desc = this.unitList[x].Descs
              break
            }
          }
          ...
          .then(ress => {
            if (ress.value) {
              ...
              var param = {
                ...
                ZoneId: this.zoneId,
                FloorPlanId: newData[0].id,
                Coordinate: 'NULL',
                Remarks: '',
                UserId: this.getDataUser().UserId
              }
              this.postEncode(this.getUrlUpdateCoordinateUnit(), param)
              .then((response) => {
    ...
    data () {
      return { 
        ...
        PI_Unit: { 
          ...
          cOption: [], 
          cMasterUrl: '',
          cDisplayColumn: 'LotNo,FloorCd,FloorPlanDescs,ZoningCd,ZoneDescs,TimeEdit',
        },
      }
    },
    created () {
    ...
    methods: {
      OnUnitChange (data) {
        this.$nextTick(() => {
            this.unitChoosed = data.id
            this.unitChoosedLabel = data.label
      ...
      changeStatus () {
        if (this.captionButton == 'Add Marker') {
          ...
          this.unitChoosed = null
          this.$refs.modalUnit.show()
        }
      },
      refreshMap () {
      ...
      getUnit () {
        ...
        this.postEncode(this.getUrlGetCoordinateUnit(), param)
        .then((response) => {
          if (response == null) return

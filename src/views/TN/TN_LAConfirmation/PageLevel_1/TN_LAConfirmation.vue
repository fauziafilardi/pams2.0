...
export default {
    data() {
        return {
            propList: {
                initialWhere: "CustomerType = 'L' AND [Status] NOT IN ('X','C') AND ContractStatus = '3' AND DeviationApprovalNo IS NULL AND LANo IS NOT NULL AND LADate IS NOT NULL AND SubPortfolioCd = '"+this.getDataUser().SubPortfolioCd+"' AND (UserID IS NULL OR (UserId='"+this.getDataUser().UserId+"' AND LAStatus = 'Y' ))",
                LineNo: 0,
                PageLevel: 1,
                TabIndex: 1
            },

						...
						<b-form :data-vv-scope="'FormScope_' + PageLevel + '_' + TabIndex" :data-vv-value-path="'FormScope_' + PageLevel + '_' + TabIndex">
                            <b-row style="padding-left:10px;">					
                                ...
                                <b-col md="6" id="col-right">
                                    <b-row>
                                      <b-col  md="12">
                                        <ABSCompute :prop="PI_nothing" v-model="M_TN_ContractDepositBilling.nothing" />
                                      </b-col>
                                    </b-row>
                                    <b-row>
                                      <b-col  md="12">
...
export default {
    ...
    methods: {
        ...
        GetDefaultDepositBilling(trxtype){
            var ttlarea = 0
            switch (this.ContractData.ChargeArea) {
                case "N":
                    ttlarea = this.isCurrency(this.ContractData.TotalNetArea, this.decimal)
                    break;
                case "N":
                    ttlarea = this.isCurrency(this.ContractData.TotalSemiGrossArea, this.decimal)
                    break;
                default:
                    ttlarea = this.isCurrency(this.ContractData.TotalNetArea, this.decimal)
                    break;
             ...
             var param = {
                ...
                var data = ress[0]
                var BrenRate = data.BaseRentalRate == '' || data.BaseRentalRate == null ? 0 : data.BaseRentalRate
                var SdepRent = data.SecurityDepositRental == '' || data.SecurityDepositRental == null ? 0 : data.SecurityDepositRental
                var SC = data.ServiceCharge == '' || data.ServiceCharge == null ? 0 : data.ServiceCharge
                var SDS = data.SecurityDepositSC == '' || data.SecurityDepositSC == null ? 0 : data.SecurityDepositSC

                if(data.TenancyChargeType=='1'){
                    this.M_TN_ContractDepositBilling.depositamt = this.isCurrency( BrenRate * ttlarea * SdepRent,this.decimal)
        ...
        this.$nextTick(() => {
            ...
            if (this.inputStatus != "VIEW") {
                ...
                .then(dt => {
                    ...
                    this.GetCreditTerms()
                    this.GetDefaultDepositBilling(data.id)
                })
            }
        })
        this.$forceUpdate()

...
export default {
    data() {
        return {
            propList: {
                initialWhere: "SubportfolioCd ='"+this.getDataUser().SubPortfolioCd+"' AND CustomerType = 'L' AND [Status] NOT IN ('X','C') AND ContractStatus = '1' AND DeviationApprovalNo IS NULL AND LOONo IS NOT NULL AND LOODate IS NOT NULL AND (UserId IS NULL OR (UserId='"+this.getDataUser().UserId+"' AND LOOStatus = 'Y'))",
                LineNo: 0,
                PageLevel: 1,
                TabIndex: 1
            },

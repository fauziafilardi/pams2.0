    ...
    methods: {
        ...
        getDataList (search) {
            var param = {
                ...
                BrandCd: this.DataRowPage1.BrandCd,
                LineNo: 2,
                InitialWhere: search && search != '' ? ("ContactId like '%" + search + "%' OR name like '%" + search + "%'") : '',
                ParamView: "'"+this.getDataUser().SubPortfolioCd+"', '"+this.DataRowPage1.BrandCd+"'" ,
                SortField: 'ContactId'
            ...
            .then( response => {
                ...
                for (var x = 0; x < this.allDataOptions.length; x++) {
                    ...
                    for (var y = 0; y < this.rightDataOptions.length; y++) {
                        if (this.allDataOptions[x].ContactId == this.rightDataOptions[y].value) {  
                            status = true
                            break
                        }
                    }
                    if (status == false) {
                        this.leftDataOptions.push({
                            value: this.allDataOptions[x].ContactId,                             
                            key: this.allDataOptions[x].ContactId + " - " + this.allDataOptions[x].name
                            // ,
        ...
        //         SubPortfolioCd: this.getDataUser().SubPortfolioCd,
        //         BrandCd: this.DataRowPage1.BrandCd,
        //         // ParamWhere: "(ContactId like '%" + search + "%' OR Name  like '%" + search + "%')"
        //     }
            ...
            .then( response => {
                ...
                for (var x = 0; x < data.length; x++) {
                    this.rightDataOptions.push({
                        value: data[x].marketingid,
                        key: data[x].marketingid + " - " + data[x].name
                    })
                }
		...
		paramFromParent(){
            ...
            // console.log(this.DataRow.BrandCd)
            this.getRightColumn('')
        },	
        M_Save() {

...
export default {
    data() {
        return {
            propList: {
                initialWhere: "ProcessControlSequenceNo IS NULL AND SubPortfolioCd = '" + this.getDataUser().SubPortfolioCd + "' AND Status = 'N' AND CustomerType = 'L'",
                LineNo: 0,
                PageLevel: 1,
                TabIndex: 1
            },
    ...
    methods: {
        ...
        M_Post() {
            ...
            data.forEach((value) => {
                dataPost.push({
                    _Message_ : "Deviation No = "+value.DeviationApprovalNo + " Confirmed ",
    ...
    mounted: function() {
        if(this.utk != undefined){
            this.propList.initialWhere = "ProcessControlSequenceNo IS NULL AND SubPortfolioCd = '" + this.getDataUser().SubPortfolioCd + "' AND Status = 'N' AND CustomerType = 'C'"
        }
        else {
            this.propList.initialWhere = "ProcessControlSequenceNo IS NULL AND SubPortfolioCd = '" + this.getDataUser().SubPortfolioCd + "' AND Status = 'N' AND CustomerType = 'L'"
        }

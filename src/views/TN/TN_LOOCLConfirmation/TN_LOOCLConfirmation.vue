...
export default {
    data() {
        return {
            propList: {
                initialWhere: "CustomerType = 'C' AND [Status] NOT IN ('X','C') AND ContractStatus = '1' AND DeviationApprovalNo IS NULL AND LOONo IS NOT NULL AND LOODate IS NOT NULL AND SubPortfolioCd = '" +this.getDataUser().SubPortfolioCd+ "' AND (UserID IS NULL OR (UserID = '" +this.getDataUser().UserId+ "' AND LOOStatus = 'Y'))",
                LineNo: 0,
                PageLevel: 1,
                TabIndex: 1
            },

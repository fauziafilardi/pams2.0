...
export default {
    data() {
        return {
            propList: {
                initialWhere: "CustomerType = 'C' AND [Status] NOT IN ('X','C') AND ContractStatus = '3' AND DeviationApprovalNo IS NULL AND LANo IS NOT NULL AND LADate IS NOT NULL AND SubPortfolioCd = '" + this.getDataUser().SubPortfolioCd + "' AND (UserID IS NULL OR (UserID = '" + this.getDataUser().UserId + "' AND LAStatus = 'Y'))",
                LineNo: 0,
                PageLevel: 1,
                TabIndex: 1
            },

                ...
                <b-col md="12" id="col-left">
                  ...
                  <b-row>
                    <b-col md="5">
                      <ABSInputSelect
                        @change="onChange"
                        :prop="PI.CreditTermsCd"
                        v-model="M_AR_DebitH.CreditTermsCd"
                      />
                    </b-col>
                    <b-col md="6">
                  ...
                  <b-row>
                    <b-col md="5">
                      <ABSInputSelect
                        @change="onChange"
                        :prop="PI.CurrencyCd"
                        v-model="M_AR_DebitH.CurrencyCd"
                      />
...
export default {
  ...
  methods: {
    ...
    M_Delete(data) {
      ...
      this.postJSON(this.postJSONMulti(), param).then(response => {
        // response from API
    ...
    beforeSave() {
      if (this.M_AR_DebitH.RunningCd == "") {
        ...
        if (this.$parent.state !== "UPDATE") {
          this.generateRunningCd().then(ress => {
            // console.log(ress)
            if (ress == null) return;
    ...
    doSave() {
      ...
      this.M_AR_DebitH.CurrencyRate = this.M_AR_DebitH.CurrencyRate
        ? this.M_AR_DebitH.CurrencyRate.replace(/,/g, "").split(".")[0]
        : 0;
      this.M_AR_DebitH.TrxAmt = this.M_AR_DebitH.TrxAmt
        ? this.M_AR_DebitH.TrxAmt.replace(/,/g, "").split(".")[0]
        : 0;
      ...
      if (this.$parent.state !== "NEW") {
        ...
        this.postJSON(this.getUrlUpdate(), this.M_AR_DebitH).then(response => {
          if (response == null) return;
        ...
        this.postJSON(this.getUrlInsert(), this.M_AR_DebitH).then(response => {
          if (response == null) return;
        ...
        if (data.length > 0) {
          ...
          this.$nextTick(() => {
            ...
            this.M_AR_DebitH.DebitDate = data.DebitDate
              ? this.momentDate(data.DebitDate)
              : data.DebitDate;
            this.M_AR_DebitH.JournalDate = data.JournalDate
              ? this.momentDate(data.JournalDate)
              : data.JournalDate;
            ...
            this.M_AR_DebitH.DepositRefundDate = data.DepositRefundDate
              ? this.momentDate(data.DepositRefundDate)
              : data.DepositRefundDate;
            this.M_AR_DebitH.BillingDueDate = data.DueDate
              ? this.momentDate(data.DueDate)
              : data.DueDate;
            ...
            EventBus.$emit(
              "ebSetValue" +
                this.PI.DebtorCd.cName +
                "_" +
                this.PageLevel +
                "_" +
                this.TabIndex,
              { data: data.DebtorCd, action: "getByID" }
            );
            EventBus.$emit(
              "ebSetValue" +
                this.PI.TrxType.cName +
                "_" +
                this.PageLevel +
                "_" +
                this.TabIndex,
              { data: data.TrxType, action: "getByID" }
            );
            this.M_AR_DebitH.CurrencyRate = this.isCurrency(data.CurrencyRate);
    ...
    onChange(data) {
      ...
      if (
        from ==
        this.PI.DebtorCd.cName + "_" + this.PageLevel + "_" + this.TabIndex
      ) {
        ...
        if (data.value && data.value.id !== "") {
          ...
          if (data.action !== "getByID" && data.action !== "manual") {
            EventBus.$emit(
              "ebSetValue" +
                this.PI.CreditTermsCd.cName +
                "_" +
                this.PageLevel +
                "_" +
                this.TabIndex,
              { data: data.value.CreditTermsCd, action: "onChange" }
            );
            ...
            EventBus.$emit(
              "ebSetValue" +
                ...
                "_" +
                this.TabIndex,
              { data: this.M_AR_DebitH.CreditTermsCd, action: "onChange" }
            );
          }
          EventBus.$emit(
            "ebSetInitWhere" +
              this.PI.AddressCd.cName +
              "_" +
              this.PageLevel +
              "_" +
              this.TabIndex,
            { data: where, default: this.M_AR_DebitH.AddressCd }
          );
        }
      }
      ...
      //TrxType
        ...
        from ==
        this.PI.TrxType.cName + "_" + this.PageLevel + "_" + this.TabIndex
      ) {
        ...
        if (data.value && data.value.id !== "") {
          ...
          if (data.action !== "getByID" && data.action !== "manual") {
            ...
            EventBus.$emit(
              "ebSetValue" +
                this.PI.CurrencyCd.cName +
                "_" +
                this.PageLevel +
                "_" +
                this.TabIndex,
              { data: data.value.CurrencyCd, action: "onChange" }
            );
            this.getCurrencyRate(
              this.M_AR_DebitH.DebitDate,
              data.value.CurrencyCd
            );
          } else {
            EventBus.$emit(
              "ebSetValue" +
                ...
                "_" +
                this.TabIndex,
              { data: this.M_AR_DebitH.CurrencyCd, action: "manual" }
            );
          }
        }
      }
      ...
      //CurrencyCd
        ...
        from ==
        this.PI.CurrencyCd.cName + "_" + this.PageLevel + "_" + this.TabIndex
      ) {
        if (data.value && data.value.id !== "") {
      ...
      //CreditTermsCd
        ...
        from ==
        this.PI.CreditTermsCd.cName + "_" + this.PageLevel + "_" + this.TabIndex
      ) {
        if (data.value && data.value.id !== "") {
      ...
      //AddressCd
        ...
        from ==
        this.PI.AddressCd.cName + "_" + this.PageLevel + "_" + this.TabIndex
      ) {
        if (data.value && data.value.id !== "") {
            ...
            if (this.PI[key].cName == el.cName) {
              if (this.$parent.state == "NEW") {
                if (this.PI[key].cType == undefined) {
                  ...
                  setTimeout(() => {
                    EventBus.$emit("ebSetValue" + el.cName, {
                      data: el.cDefault,
                      action: "setDefault"
                    });
                  }, 500);

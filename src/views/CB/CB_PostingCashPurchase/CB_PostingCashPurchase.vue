...
export default {
    data() {
        return {
            propList: {
                initialWhere: "SubPortfolioCd = '" + this.getDataUser().SubPortfolioCd + "' AND Status ='B' AND EntryType = 'C'",
                LineNo: 0,
                PageLevel: 1,
                TabIndex: 1
            },
        ...
        M_Post() {
            ...
            data.forEach((value) => {
                dataPost.push({
                    ...
                    CheckedBatchNo: value.BatchNo.toString(),
                    PostBy: this.getDataUser().UserId,
                    BudgetOwnerApprovalErrorMessage_Output: '',
                    _Message_:'Batch No : '+value.BatchNo.toString()
                })
            })

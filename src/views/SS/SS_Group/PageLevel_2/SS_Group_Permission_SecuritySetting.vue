                ...
                <b-row v-show="isShow" style="padding-left: 10px; padding-bottom: 10px; !important;">
                    <b-col md="12" id="col-mid" class="bColMasterForm">
                        <b-container class="bv-example-row">
                            <div v-for="(module, indexModule) in allModules" v-bind:key="module.ModuleSeq">
                                <!-- Module -->
                                <b-row :no-gutters="true">
                                    <b-col cols="auto"> <span @click="expand('M', module.ModuleSeq, indexModule)" class="module" :id="'module_'+module.ModuleSeq"><font-awesome-icon :icon="module.icon?module.icon:'plus-circle'" class="icon-expand" /></span> </b-col>
                                ...
                                <b-row :no-gutters="true" v-show="showEvent == module.ModuleSeq">
                                    <b-container class="bv-example-row" v-show="onFilteredEvents.length > 0">
                                        <div v-for="(event, indexEvent) in onFilteredEvents" v-bind:key="event.EventSeq">
                                            <!-- Event -->
                                            <b-row style="margin-left: 26px" :no-gutters="true">
                                                <b-col cols="auto"> <span @click="expand('E', event.EventSeq, indexEvent)" class="event" :id="'event_'+event.EventSeq"><font-awesome-icon :icon="event.icon?event.icon:'plus-circle'" class="icon-expand" /></span> </b-col>
                                            ...
                                            <b-row :no-gutters="true" v-show="showOption == event.EventSeq">
                                                ...
                                                <b-container class="bv-example-row" v-show="onFilteredEvents[indexEvent].options.length > 0">
                                                    ...
                                                    <tr v-for="(option, indexOption) in onFilteredEvents[indexEvent].options" v-bind:key="option.OptionSeq">
                                                        <!-- Option -->
                                                        ...
                                                        <td class="col-auto" style="text-align:center !important"> <input :disabled="!canUpdate" type="checkbox" name="cb_optionStatus" :id="'OS_Post_'+option.OptionSeq" :checked="option.print" @click="Update('OS_Post', option.OptionSeq)" v-model="option.post" > </td>
                                                        <!-- End Option -->
                                                    </tr>
                                                    <!-- </div> -->
                                                    </table>
        ...
        getGroupSubPortfolio () {
            ...
            .then(response => {
                ...
                this.$nextTick(() => {
                    ...
                    //     var idx = allO.map(o => o.OptionSeq.toString()).indexOf(data[dt].optionseq.toString())
                    //     if (idx > -1) {
                    ...
                    //         var idxEvent = allE.map(e => e.EventSeq.toString()).indexOf(this.allOptions[idx].EventSeq.toString())
                    //         this.allEvents[idxEvent].set = true

                    //         var idxModule = allM.map(m => m.ModuleSeq.toString()).indexOf(this.allOptions[idx].ModuleSeq.toString())
                    //         this.allModules[idxModule].set = true
                    //     }
                    // }
                    data.forEach(element => {
                        var idx = allO.map(el => { return el.OptionSeq.toString() }).indexOf(element.optionseq.toString())
                        if (idx > -1) {
                            this.allOptions[idx].set = true
                            ...
                            var idxEvent = allE.map(el => { return el.EventSeq.toString() }).indexOf(this.allOptions[idx].EventSeq.toString())
                            this.allEvents[idxEvent].set = true

                            var idxModule = allM.map(el => { return el.ModuleSeq.toString() }).indexOf(this.allOptions[idx].ModuleSeq.toString())
                            this.allModules[idxModule].set = true
                        }
                    })
                    this.canUpdate = true
